...
async function getPTMQuestions(body) {
    ...
    try {
        ...
        query = "SELECT" +
            " `web_ptm_master`.`ptm_id`            AS `ptm_id`," +
            "  `web_ptm_master`.`ptm_date`          AS `ptm_date`," +
            "  `web_ptm_master`.`ptm_agenda`        AS `ptm_agenda`," +
            "  `web_ptm_master`.`status`            AS `status`," +
            "  `web_ptm_questions`.`question_order` AS `question_order`," +
            "  `web_ptm_questions`.`question`       AS `question`," +
            " `web_ptm_questions`.`remarks`        AS `remarks`" +
            "FROM " +
            "       `web_ptm_master`" +
            "   INNER JOIN " +
            "  `web_ptm_questions` USING(ptm_id)" +
            "  WHERE `ptm_id` = '" + ptm_id + "';";
        ...
        if (queryModel.status == 1) {
            ...
            if (queryModel.fetchedRows == 0) {
                ...
                // new PTM ptm_master, question master
                //new form feedback details  se aayengi
                //query 
                query = "SELECT" +
                    " `web_ptm_master`.`ptm_id`            AS `ptm_id`," +
                    "  `web_ptm_master`.`ptm_date`          AS `ptm_date`," +
                ...
                query = "SELECT" +
                    ...
                    "`web_ptm_master`.`ptm_agenda`      AS `ptm_agenda`," +
                    "`web_ptm_master`.`status`          AS `status`," +
                    "`web_ptm_details`.`question_order` AS `question_order`," +
                    "`web_ptm_details`.`question`       AS `question`" +
                    "FROM " +
                    " web_ptm_master" +
                    "INNER JOIN" +
                    "  web_ptm_details USING(ptm_id) " +
                    " WHERE ptm_id = 'PTM-BCA-2020-03-29';";
